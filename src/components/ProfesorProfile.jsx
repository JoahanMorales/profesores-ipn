...
const ProfesorProfile = () => {
  ...
  const [likeAnimId, setLikeAnimId] = useState(null); // 'evalId-like' or 'evalId-dislike'
  ...
  const cargarProfesor = async () => {
    ...
    if (profesorResult.success && profesorResult.data) {
      ...
      if (evalResult.success) {
        ...
        // Cargar likes en batch (en paralelo)
        if (evals.length > 0) {
          const evalIds = evals.map(e => e.id);
          const likesPromise = obtenerLikesBatch(evalIds, slug);
          const misLikesPromise = user?.id
            ? obtenerMisLikesBatch(evalIds, String(user.id), slug)
            : Promise.resolve({});

          const [likes, mis] = await Promise.all([likesPromise, misLikesPromise]);
          setLikesMap(likes);
          setMisLikes(mis);
        }
      }
  ...
  const handleLike = async (evalId, tipo) => {
    ...
    if (likingId === evalId) return;

    // Optimistic update — instant visual feedback
    const prevMisLikes = { ...misLikes };
    const prevLikesMap = { ...likesMap };
    const prevTipo = misLikes[evalId];

    // Calculate new state optimistically
    const newMisLikes = { ...misLikes };
    const newLikesMap = { ...likesMap };
    const current = { ...(newLikesMap[evalId] || { likes: 0, dislikes: 0 }) };

    if (prevTipo === tipo) {
      // Toggle off
      delete newMisLikes[evalId];
      if (tipo === 'like') current.likes = Math.max(0, current.likes - 1);
      ...
      newMisLikes[evalId] = tipo;
      if (tipo === 'like') current.likes++;
      else current.dislikes++;
    }
    newLikesMap[evalId] = current;

    // Apply instantly + trigger pop animation
    setMisLikes(newMisLikes);
    setLikesMap(newLikesMap);
    setLikingId(evalId);
    setLikeAnimId(`${evalId}-${tipo}`);
    setTimeout(() => setLikeAnimId(null), 400);

    // Fire-and-forget server call, rollback on error
    try {
      const resultado = await toggleLikeEvaluacion(evalId, String(user.id), tipo, slug);
      if (resultado === undefined) {
        // Server error — rollback
        setMisLikes(prevMisLikes);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
              ...
              <>
                ...
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg z-20 py-2 animate-fade-in">
                  ...
                  <a
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  >
                    <svg className="w-4 h-4 text-[#229ED9]" fill="currentColor" viewBox="0 0 24 24"><path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0a12 12 0 00-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                    Telegram
                  </a>
                  <a
        ...
        {/* Professor Info Card */}
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 sm:p-8 mb-6 sm:mb-8 overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-4">
                ...
                {profesor.total_evaluaciones >= 3 && (
                  <div className="flex-shrink-0" title="Profesor con múltiples evaluaciones">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-r from-ipn-guinda-700 via-ipn-guinda-400 to-ipn-guinda-700 animate-badge-shimmer flex items-center justify-center shadow-sm">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-gray-700 dark:text-gray-300">
                  ...
                  <span className="text-base sm:text-lg">{profesor.total_evaluaciones || 0} evaluaciones</span>
                </div>
            ...
            {/* Rating Box */}
            <div className={`${getRatingBg(profesor.calificacion_promedio)} rounded-2xl p-4 sm:p-6 text-center md:min-w-[200px] border border-gray-100 dark:border-gray-600`}>
              <div className={`text-4xl sm:text-5xl font-extrabold mb-1 ${getRatingColor(profesor.calificacion_promedio)}`}>
          ...
          {/* CTA Buttons */}
          <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row gap-3">
            <button
              onClick={() => {
                if (!isAuthenticated()) {
              ...
              className="w-full sm:w-auto px-6 sm:px-8 py-3 text-sm sm:text-base font-semibold text-white bg-ipn-guinda-900 dark:bg-ipn-guinda-700 rounded-xl hover:bg-ipn-guinda-800 dark:hover:bg-ipn-guinda-600 transition-all shadow-lg shadow-ipn-guinda-900/20 hover:shadow-xl hover:-translate-y-0.5 text-center"
            >
        ...
        <section>
          ...
          {loading ? (
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 sm:p-12 text-center">
            ...
            <div className="space-y-4">
              {evaluaciones.map((evaluacion) => (
                <div
                  key={evaluacion.id}
                  className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 sm:p-6 overflow-hidden"
                >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {evaluacion.recomendado && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-ipn-guinda-100 dark:bg-ipn-guinda-900/30 text-ipn-guinda-900 dark:text-ipn-guinda-300">
                            ✓ Recomendado
                          </span>
                        )}
                        {evaluacion.asistencia_obligatoria && (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-ipn-guinda-50 dark:bg-ipn-guinda-950/20 text-ipn-guinda-700 dark:text-ipn-guinda-400">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-ipn-guinda-600 to-ipn-guinda-900 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-md flex-shrink-0">
                  ...
                  <div className="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 relative">
                    ...
                    <button
                      ...
                      className={`inline-flex items-center justify-center gap-1 min-w-[48px] px-2.5 py-1 rounded-lg text-xs font-medium tabular-nums transition-all ${
                        misLikes[evaluacion.id] === 'like'
                          ? 'bg-ipn-guinda-100 dark:bg-ipn-guinda-900/40 text-ipn-guinda-900 dark:text-ipn-guinda-300 ring-1 ring-ipn-guinda-300 dark:ring-ipn-guinda-700'
                          : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-ipn-guinda-50 dark:hover:bg-ipn-guinda-900/20 hover:text-ipn-guinda-700'
                      }`}
                    >
                      <svg className={`w-3.5 h-3.5 flex-shrink-0${likeAnimId === `${evaluacion.id}-like` ? ' animate-like-pop' : ''}`} fill="currentColor" viewBox="0 0 20 20"><path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z" /></svg>
                      <span className="min-w-[1ch]">{(likesMap[evaluacion.id]?.likes || 0) > 0 ? likesMap[evaluacion.id].likes : ''}</span>
                    </button>
                    <button
                      ...
                      className={`inline-flex items-center justify-center gap-1 min-w-[48px] px-2.5 py-1 rounded-lg text-xs font-medium tabular-nums transition-all ${
                        misLikes[evaluacion.id] === 'dislike'
                          ? 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 ring-1 ring-gray-300 dark:ring-gray-500'
                    ...
                    >
                      <svg className={`w-3.5 h-3.5 flex-shrink-0${likeAnimId === `${evaluacion.id}-dislike` ? ' animate-like-pop' : ''}`} fill="currentColor" viewBox="0 0 20 20"><path d="M18 9.5a1.5 1.5 0 11-3 0v-6a1.5 1.5 0 013 0v6zM14 9.667v-5.43a2 2 0 00-1.106-1.79l-.05-.025A4 4 0 0011.057 2H5.64a2 2 0 00-1.962 1.608l-1.2 6A2 2 0 004.44 12H8v4a2 2 0 002 2 1 1 0 001-1v-.667a4 4 0 01.8-2.4l1.4-1.866a4 4 0 00.8-2.4z" /></svg>
                      <span className="min-w-[1ch]">{(likesMap[evaluacion.id]?.dislikes || 0) > 0 ? likesMap[evaluacion.id].dislikes : ''}</span>
                    </button>
