-- ============================================
-- MIGRACIÓN: Filtro de contenido ofensivo server-side
-- ============================================
-- Fecha: 2026-02-20
-- Descripción:
--   Agrega validación de palabras prohibidas directamente
--   en el RPC crear_evaluacion_segura. No se puede saltar
--   desde el frontend.
-- ============================================

-- 1. Función auxiliar para verificar contenido ofensivo
CREATE OR REPLACE FUNCTION public.contiene_contenido_ofensivo(p_texto TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  v_texto := lower(
    translate(p_texto,
      'ÁÉÍÓÚáéíóúÑñÜüÄËÏÖÜäëïöü',
      'AEIOUaeiouNnUuAEIOUaeiou')
  );

  -- Revisar cada palabra prohibida
  FOREACH v_palabra IN ARRAY v_palabras
  LOOP
    IF v_texto LIKE '%' || v_palabra || '%' THEN
      RETURN TRUE;
    END IF;
  END LOOP;
...
-- 2. Actualizar RPC para incluir verificación de contenido
